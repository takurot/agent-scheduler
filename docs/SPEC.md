...
`close_issue: false`（既定）：
PR作成時にIssueを自動クローズしない。PR本文にはauto-close keyword（`Closes #123`等）を含めず、手動レビューまでIssueをオープンにしておく。

`close_issue: true`：
PRがbase branchへマージされた際に、GitHubの標準機能によって対象Issueが自動クローズされるよう連携する。具体的には、PR本文末尾に `Closes #<issue>` を含める。
この方式により、PR作成時点ではIssueはオープンのまま維持され、未マージ状態での誤クローズを防ぎつつ、レビュー後のマージ完了と同時に安全に自動クローズされる。

`subsched run`は起動時に、native実行・push・create_pr・close_issueの実効policyを1行で表示する。

GitHub CLIにはPR作成機能があり、PR bodyからIssueを関連付けられる。

推奨PR body（`close_issue: false`の場合）：
...
推奨PR body（`close_issue: true`の場合）：
...
`## Summary`セクションの内容は`extract_pr_summary()`（#249）が生成する。`task.worktree`が
判明している場合、まずworktreeの`.ai/handoffs/<issue>.md`を`parse_semantic_handoff()`で
パースし、`## Completed`が空でも既定のbootstrap文言（例: `- Task bootstrapped`）でもなければ、
それを`### Completed Changes`として、また`## Decisions`が同様に有意であれば`### Key Decisions`
として整形して使う。handoffが無い・未記入の場合は`git log <base_branch>..HEAD`のcommit
subject/bodyを`### Commits`として整形し代替とする。どちらも得られない場合は`task.title`へ
安全にfallbackする。`task.worktree`が不明な場合は常に`task.title`を使う。
いずれの経路で生成した文字列も、下流の`build_pr_body()`が他のsummary同様に`_sanitize_pr_section()`
（secret redaction、auto-close keyword strip、文字数上限）へ通すため、`Implements work for #N.`
prefixの維持やauto-close防止の不変条件は崩れない。

PR bodyは公開境界であるため、SummaryとVerificationの両方へsecret redactionと文字数上限を
適用する。verification commandが見つからない場合のstderrも、要約へ含める前に同じredactionと
上限を適用する。上流でsanitize済みでも、PR作成直前に再適用して新しい入力経路からの漏えいを
防ぐ。
...
- `github.completion.close_issue: false`（既定）：Issueは自動クローズされずオープンのまま維持される。
- `github.completion.close_issue: true`：PR本文に `Closes #<issue>` が含まれるため、PRがマージされた時点でGitHubにより自動クローズされる。
...
に限定する（`github.completion.close_issue` は `true`/`false` をサポートし既定値は `false`）。未実装の値を受理して無視せず、config load時に
...
OpenAI API endpoint and denies non-443, IP-literal, and unmatched destinations. The proxy tunnels
TLS without interception. Operators must test the intended Linux worker image through the proxy;
