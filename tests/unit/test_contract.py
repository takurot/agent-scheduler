...
def test_bootstrap_task_files_does_not_create_repository_instruction_files(
    tmp_path: Path,
) -> None:
    task = Task.from_issue(Issue(number=103, title="Support timeout"))
...
def test_bootstrap_task_files_preserves_a_single_repository_instruction_file(
    tmp_path: Path,
) -> None:
    for existing_name, missing_name in (
        ...
        worktree = tmp_path / existing_name.removesuffix(".md").lower()
        worktree.mkdir()
        content = f"# User-owned {existing_name}\n"
        (worktree / existing_name).write_text(content, encoding="utf-8")
        task = Task.from_issue(Issue(number=103, title="Support timeout"))

        bootstrap_task_files(worktree, task)

        assert (worktree / existing_name).read_text(encoding="utf-8") == content
        assert not (worktree / missing_name).exists()
